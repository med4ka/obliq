"""Storage of BPS inflation YoY into PostgreSQL.
...
formal migration. Since `fetched_at` changes on every run, this is also how we
record when a value was last retrieved.
"""
    ...
    with engine.begin() as conn:
        for obs in observations:
            conn.execute(
                UPSERT_SQL,
                {
                    "indicator_type": INDICATOR_TYPE_YOY,
                    "observation_date": obs.observation_date,
                    "value": obs.value,  # Decimal stays Decimal through the ORM driver
                    "source": SOURCE_BPS,
                    "fetched_at": fetched_at,
                },
            )
            count += 1
    logger.info("BPS upsert selesai: %d observasi (indicator=%s source=%s)", count, INDICATOR_TYPE_YOY, SOURCE_BPS)
    return count