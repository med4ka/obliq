...
from pipeline.fetchers.bps import BpsYearResponse, fetch
...
class TestYoyTransform:
    def test_yoy_hlm_dari_ihk_setahun_lalu(self) -> None:
        # IHK bulan Jan - Jan tahun lalu. Pilih nilai yang mudah dicheck.
    ...
    def test_yoy_definisi_umum(self) -> None:
        # YoY = (IHK_now - IHK_12bln_lalu) / IHK_12bln_lalu * 100
        # (121 - 110) / 110 * 100 = 10.00 persis
        data = [
    ...
    def test_titik_tanpa_pasangan_setahun_lalu_skip(self) -> None:
        # 2022 tanpa data 2021 -> semua titik 2022 harus di-skip (bukan
        # interpolasi), dengan log (tidak silent).
...
class TestFetchYearSelection:
    ...
    @staticmethod
    def _stub_response(year: int, th_id: int) -> BpsYearResponse:
        return BpsYearResponse(year=year, th_id=th_id, body={"datacontent": {}})
    ...
    def test_select_subset_tahun(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        def fake_fetch(api_key: str, year: int, th_id: int) -> BpsYearResponse:
        ...
        assert calls == [(2022, 122), (2023, 123)]  # th_id ikut benar
    ...
    def test_skip_tahun_tidak_didukung(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 2019 (di-support) dan 2024 (belum dibuka) -> hanya yang didukung
